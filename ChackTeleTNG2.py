...
class ReportConfig:
    ...
    PARAM_PHASE_DEFINITIONS = {
        '執筆フェーズ': [
            # 元の「準備フェーズ」の項目
            '雛形/パラメータ決定', '情報収集', '雛形(PV)B.U.', '課題抽出/QA発出', '課題抽出/QA発出（中断）',
            '課題抽出/QA発出(再開）', '非互換影響確認', '雛形(PV)認識合わせ', 'ヒアリング項目作成', '非互換確認',
            '環境準備', 'APヒアリング', '雛形入手or作成', '(雛形すり合わせ）', '非互換確認／修正',
            '雛形（集配信定義除く）B.U.', '雛形作成',
            # 元の「パラメータシート作成フェーズ」の項目
            'パラメータ仮埋め', 'パラメータ仮埋め（コア）','パラメータシート修正', 'パラメタ見直し', '非互換/変更要件反映',
            '中間すり合わせ', 'パラメータ見直し', 'パラメータ仮決め', 'パラメータ仮決め（ジョブ除く）', 'パラメータ入力',
            '作成', 'パラメータ修正'
        ],
        'T内Revフェーズ': ['T内Rev', 'T'],
        'GLRevフェーズ': ['GLRev', 'GL'],
